...
**Tablo 3. CC1352R flash yerleşimi**
...
| Metadata alanı | `0x00052000` | `0x00002000` | Üretim seviyesinde sürüm, boyut, CRC ve geçerlilik bilgisinin tutulabileceği ayrılmış alan | Bu çalışmada otomatik metadata yazma gerçeklenmemiştir |
| Recovery alanı | `0x00054000` | `0x00002000` | Üretim seviyesinde kurtarma veya fallback bilgileri için ayrılmış alan | Otomatik recovery/rollback gelecek çalışma olarak bırakılmıştır |
...
![CC1352R önerilen flash yerleşimi](figures/flash-layout.png){width=95%}
...
![CC1352R boot karar akışı](figures/boot-chain.png){width=95%}
...
![İkinci firmware imajı için önerilen yerleşim ve aktivasyon akışı](figures/activation-strategy.png){width=95%}

Yukarıdaki aktivasyon akışı, ikinci firmware imajının doğrudan fallback alanının üzerine yazılmadığını; page 0 user/update image alanına yerleştirildiğini, ardından doğrulama ve üretim seviyesinde metadata güncellemesiyle boot sürecine aday hale getirilebileceğini göstermektedir. Bu yaklaşım, hatalı bir firmware imajının cihazı kullanılamaz hale getirmesini önlemek için tercih edilmiştir.
...
Bu yaklaşım aynı zamanda fallback imkânı sağlar. Eğer yeni firmware eksik, bozuk veya uyumsuz ise sistemin mevcut `old-firmware` ile çalışmaya devam etmesi mümkündür. Bu nedenle güvenli firmware güncelleme tasarımında page 0 user/update alanı, metadata kontrolü ve bootloader kararı birlikte düşünülmelidir.
